...
const schema = z.object({
  ...
  telefono: z.string().min(10, "Ingresa un número válido de 10 dígitos").max(15),
...
const destinos = ["Cancún", "Puerto Vallarta", "Los Cabos", "Mazatlán", "Cozumel", "Huatulco", "Otro destino"];
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        <div className="contacto-content grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          {/* Left */}
          <div>
            <p className="text-teal text-xs font-body font-medium uppercase tracking-widest mb-4">Cotiza Gratis</p>
            <h2
            ...
            <div className="bg-white border border-border rounded-xl p-6 mb-5 shadow-sm">
              <p className="font-body text-muted text-xs uppercase tracking-wider mb-3">Llámanos directamente</p>
              <a
                href="tel:+528002288377"
                className="flex items-center gap-3 text-cream font-display font-bold text-2xl hover:text-coral transition-colors duration-200"
              >
            ...
            {sent ? (
              <div className="text-center py-10">
                ...
                <p className="font-body text-cream-dim text-base leading-relaxed">
                  Un asesor se pondrá en contacto contigo muy pronto. También puedes llamarnos al{" "}
                  <a href="tel:+528002288377" className="text-coral hover:underline">800 228 8377</a>.
                </p>
              </div>
            ) : (
              <form onSubmit={handleSubmit(onSubmit)} noValidate>
                <h3 className="font-display font-bold text-cream text-xl mb-6">Solicita tu Cotización Gratuita</h3>
                <div className="flex flex-col gap-5">

                  {/* Nombre */}
                  <div>
                    <label htmlFor="nombre" className="block font-body text-sm text-cream-dim mb-1.5">
                      Nombre completo <span className="text-coral">*</span>
                    </label>
                    <input
                      id="nombre" type="text" autoComplete="name" placeholder="Tu nombre"
                      ...
                      {...register("nombre")}
                    />
                    {errors.nombre && <p id="nombre-error" className="mt-1.5 text-xs font-body text-coral" role="alert">{errors.nombre.message}</p>}
                  </div>

                  {/* Teléfono */}
                  <div>
                    <label htmlFor="telefono" className="block font-body text-sm text-cream-dim mb-1.5">
                      Teléfono <span className="text-coral">*</span>
                    </label>
                    <input
                      id="telefono" type="tel" autoComplete="tel" placeholder="55 1234 5678"
                      className="w-full bg-bg border border-border rounded-md px-4 py-3 font-body text-cream text-sm placeholder:text-muted focus:border-coral focus:outline-none transition-colors duration-200"
                      aria-describedby={errors.telefono ? "telefono-error" : undefined}
                      {...register("telefono")}
                    />
                    {errors.telefono && <p id="telefono-error" className="mt-1.5 text-xs font-body text-coral" role="alert">{errors.telefono.message}</p>}
                  </div>

                  {/* Destino */}
                  <div>
                    <label htmlFor="destino" className="block font-body text-sm text-cream-dim mb-1.5">
                      Destino de interés <span className="text-coral">*</span>
                    </label>
                    <select
                      id="destino" defaultValue=""
                      ...
                      {...register("destino")}
                    >
                      <option value="" disabled className="text-muted">Selecciona un destino</option>
                      {destinos.map((d) => <option key={d} value={d} className="bg-white">{d}</option>)}
                    </select>
                    {errors.destino && <p id="destino-error" className="mt-1.5 text-xs font-body text-coral" role="alert">{errors.destino.message}</p>}
                  </div>

                  {/* Mensaje */}
                  <div>
                    <label htmlFor="mensaje" className="block font-body text-sm text-cream-dim mb-1.5">
                      ¿Algo más? <span className="text-muted">(opcional)</span>
                    </label>
                    <textarea
                      id="mensaje" rows={3}
                      placeholder="Número de personas, fechas, presupuesto..."
                  ...
                  {error && <p className="text-xs font-body text-coral text-center" role="alert">Ocurrió un error. Por favor llámanos al 800 228 8377.</p>}

                  <button
                    type="submit" disabled={sending}
                    className="w-full bg-coral hover:bg-coral-hover disabled:opacity-60 disabled:cursor-not-allowed text-white font-body font-medium text-base py-4 rounded-md transition-colors duration-200"
                  >
